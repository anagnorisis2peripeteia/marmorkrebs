...
Marmorkrebs uses `stryker-cxx` as the C++ provider. PR skills and new profiles should use `--tool stryker-cxx`; embedded C++ mutation is historical and not the supported gate path. Marmorkrebs preserves native provider evidence including mutation level, enabled mutators, execution backend fallback, source precision, scheduler, compile pruning, lifecycle, artifact placement, build graph, and `parity` / `execution.parity` eight-gap Mull/Stryker.NET metadata. Use `--parity-profile review` or `--parity-profile strict` when a review should fail on incomplete parity evidence. See [docs/stryker-cxx-spec.md](docs/stryker-cxx-spec.md) for the standalone CLI, report schema, milestones, and Marmorkrebs adapter plan.
